...
describe("Staking", function () {
  ...
  describe("Check claim", () => {
    it("It shold be claim success", async function () {
    ...
    it("It shold be claim fail because pause", async function () {
      await nft.connect(addr1).safeMint(addr1.address);
      await nft.connect(addr1).setApprovalForAll(staking.address, true);
    ...
    it("It shold be claim fail because invalid signature", async function () {
      await nft.connect(addr1).safeMint(addr1.address);
      await nft.connect(addr1).setApprovalForAll(staking.address, true);
  ...
  describe("Check set reward token and set collection ", () => {
    ...
    it("It should be revert because not owner", async () => {
      await expect(
        staking.connect(addr1).setRewardToken(rewardToken.address)
      ).to.be.reverted;

      await expect(
        staking.connect(addr1).setCollection(rewardToken.address)
      ).to.be.reverted;
    })
  ...
  it("Check re initialize: It should be reverted", async () => {
    await expect(
      staking.connect(addr1).initialize()
    ).to.be.reverted
  ...
  describe("Check _authorizeUpgrade", () => {
    ...
    it("Address after upgrade should be not change", async () => {
      ...
      const stakingV2 = await upgrades.upgradeProxy(staking.address, Staking); // just tesst
