...
describe("Staking", function () {
  ...
  describe("Check claim", () => {
    it("It should be claim success", async function () {
      ...
      await staking.connect(addr1).claimReward(0, 10000, 1, validSignature);

      await expect(
        staking.claimReward(0, 10000, 1, validSignature)
    ...
    it("It should be claim fail because pause", async function () {
      ...
      await expect(
        staking.connect(addr1).claimReward(0, 10000, 1, validSignature)
      ).to.be.reverted;
    });
    it("It should be revert because invalid signature", async function () {
      ...
      await staking.pause();
      await expect(
        staking.connect(addr1).claimReward(0, 1000, 1, validSignature)
      ).to.be.reverted;
    });
  });
  ...
  describe("Check unstake", () => {
    it("It should be unstake success", async function () {
      ...
      await staking.connect(addr1).unStake(0, 1000000, 1, validSignature);
    ...
    it("It should be unstake revert because pause", async function () {
      ...
      await staking.pause();
      await expect(staking.connect(addr1).unStake(0, 1000000, validSignature))
        .to.be.reverted;
    });
    ...
    it("It should be unstake revert because invalid signature", async function () {
      ...
      await expect(staking.connect(addr1).unStake(0, 100000, invalidSignature))
        .to.be.reverted;

      await staking.pause();
      await expect(staking.connect(addr1).unStake(0, 100000, validSignature)).to
        .be.reverted;
    });
  });
